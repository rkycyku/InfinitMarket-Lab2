...
import ShtoKompanit from './ShtoStafin';
import Mesazhi from '../../../components/Mesazhi';
import EditoKompanin from './EditoRoletStafit';
...
function Stafi(props) {
  ...
  useEffect(() => {
    const shfaqKompanit = async () => {
      try {
        setLoading(true);
        const listaEStafit = await axios.get('https://localhost:7251/api/Perdoruesi/Stafi/ShfaqStafin', authentikimi);
        setStafi(
          listaEStafit.data.map((k) => ({
            ID: k.userID,
            'Emri': k.emri,
            'Mbiemri': k.mbiemri,
            'Email': k.email,
