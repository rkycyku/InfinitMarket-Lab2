...
function Satellite() {
  ...
  useEffect(() => {
    const shfaqSatelitet = async () => {
      try {
        const satelitet = await axios.get('https://localhost:7251/api/MbrojtjaEProjektit/Satellite/ShfaqSatelitet');
        setSatelitet(
          satelitet.data.map((k) => ({
            ID: k.satelliteId,
            'Emri Satelitit': k.name,
            Planeti: k.planet && k.planet.name
          }))
        );
      } catch (err) {
        console.log(err);
  ...
  useEffect(() => {
    const shfaqPlanetet = async () => {
      try {
        const planetet = await axios.get('https://localhost:7251/api/MbrojtjaEProjektit/Planet/ShfaqPlanetet');
  ...
  useEffect(() => {
    const vendosTeDhenat = async () => {
      try {
        const teDhenatSatelitit = await axios.get(`https://localhost:7251/api/MbrojtjaEProjektit/Satellite/ShfaqSatelitinNgaID?SatelitiId=${id}`);
  ...
  const handleShtoSatelitin = async () => {
    await axios
      .post('https://localhost:7251/api/MbrojtjaEProjektit/Satellite/ShtoSatelitin', {
        name: name,
        planetId: planet
      })
      .then(() => {
  ...
  const handlePerditesoSatelitin = async () => {
    await axios
      .put(`https://localhost:7251/api/MbrojtjaEProjektit/Satellite/PerditesoSatelitin?SatelitiId=${id}`, sateliti)
      .then(() => {
        setEdito(false);
        setPerditeso(Date.now());
      })
  ...
  const handleFshijsatelitin = async () => {
    await axios
      .delete(`https://localhost:7251/api/MbrojtjaEProjektit/Satellite/FshijSatelitin?SatelitiId=${id}`)
      .then(() => {
        setFshij(false);
        setPerditeso(Date.now());
      })
      ...
      {shto && (
        <Modal className="modalEditShto" show={shto} onHide={() => setShto(false)}>
          ...
          <Modal.Footer>
            ...
            <Button style={{ backgroundColor: '#009879', border: 'none' }} onClick={handleShtoSatelitin}>
              Shto Satelitin
            </Button>
          </Modal.Footer>
        </Modal>
      ...
      {edito && (
        <Modal className="modalEditShto" show={edito} onHide={() => setEdito(false)}>
          ...
          <Modal.Body>
            <Form>
              <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
                ...
                <Form.Control type="text" value={sateliti.satelliteId} disabled />
              </Form.Group>
              <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
                <Form.Label>Emri Satelitit</Form.Label>
                <Form.Control
          ...
          <Modal.Footer>
            ...
            <Button style={{ backgroundColor: '#009879', border: 'none' }} onClick={handlePerditesoSatelitin}>
      ...
      {fshij && (
        <Modal show={fshij} onHide={() => setFshij(false)}>
          ...
          <Modal.Footer>
            ...
            <Button variant="danger" onClick={handleFshijsatelitin}>
