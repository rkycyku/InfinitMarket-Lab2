...
function Planet() {
  ...
  useEffect(() => {
    const vendosTeDhenat = async () => {
      try {
        const teDhenatPlaneti = await axios.get(`https://localhost:7251/api/MbrojtjaEProjektit/Planet/ShfaqPlanetinNgaID?PlanetiId=${id}`);
  ...
  const handleShtoPlanetin = async () => {
    await axios
      .post('https://localhost:7251/api/MbrojtjaEProjektit/Planet/ShtoPlanetin', {
        name: name,
        type: type
      })
      .then(() => {
  ...
  const handlePerditesoPlanetin = async () => {
    await axios
      .put(`https://localhost:7251/api/MbrojtjaEProjektit/Planet/PerditesoPlanetin?PlanetiId=${id}`, planeti)
      .then(() => {
        setEdito(false);
        setPerditeso(Date.now());
      })
  ...
  const handleFshijPlanetin = async () => {
    await axios
      .delete(`https://localhost:7251/api/MbrojtjaEProjektit/Planet/FshijPlanetin?PlanetiId=${id}`)
      .then(() => {
        setFshij(false);
        setPerditeso(Date.now());
      })
          ...
          <Modal.Body>
            <Form>
              ...
              <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
                ...
                <Form.Control onChange={(e) => handleTypeChange(e.target.value)} value={type} type="text" placeholder="Tipi Planetit" />
              </Form.Group>
            </Form>
          </Modal.Body>
          <Modal.Footer>
            ...
            <Button style={{ backgroundColor: '#009879', border: 'none' }} onClick={handleShtoPlanetin}>
              Save Changes
            </Button>
          </Modal.Footer>
        </Modal>
          ...
          <Modal.Body>
            <Form>
              ...
              <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
                ...
                <Form.Control onChange={(e) => handleType(e.target.value)} value={planeti.type} type="text" placeholder="Tipi Planetit" />
              </Form.Group>
            </Form>
          </Modal.Body>
          <Modal.Footer>
            ...
            <Button style={{ backgroundColor: '#009879', border: 'none' }} onClick={handlePerditesoPlanetin}>
