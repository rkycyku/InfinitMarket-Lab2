...
function EditoKompanin(props) {
    ...
    useEffect(() => {
        const ShfaqKategorine = async () => {
            try {
                const kategoria = await axios.get(`https://localhost:7251/api/Produktet/Kategoria/shfaqKategorinSipasIDs?id=${props.id}`, authentikimi);
    ...
    function handleSubmit() {
        axios.put(`https://localhost:7251/api/Produktet/Kategoria/perditesoKategorin?id=${kategoria.kategoriaId}`, kategoria, authentikimi)
            .then(x => {
