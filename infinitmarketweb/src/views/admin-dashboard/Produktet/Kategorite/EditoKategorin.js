...
function EditoKompanin(props) {
  ...
  useEffect(() => {
    const vendosKategorite = async () => {
      try {
        const kategorite = await axios.get(`https://localhost:7251/api/Kategoria/shfaqKategorit`, authentikimi);
  ...
  useEffect(() => {
    const ShfaqKategorine = async () => {
      try {
        const kategoria = await axios.get(
          `https://localhost:7251/api/Produktet/Kategoria/shfaqKategorinSipasIDs?id=${props.id}`,
          authentikimi
        );
  ...
  function handleSubmit() {
    axios
      .put(`https://localhost:7251/api/Produktet/Kategoria/perditesoKategorin?id=${kategoria.kategoriaId}`, kategoria, authentikimi)
      .then((x) => {
    ...
    <>
      ...
      {kontrolloKategorine && (
        <Modal size="sm" show={kontrolloKategorine} onHide={() => setKontrolloKategorine(false)}>
          ...
          <Modal.Body>
            <span style={{ fontSize: '10pt' }}>Nje kategori me te njejtin emer ekziston ne sistem!</span>
            <br />
