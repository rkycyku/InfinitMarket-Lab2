...
function PerditesoStatusinPorosis(props) {
    ...
    useEffect(() => {
        const vendosPorosine = async () => {
            try {
                const porosia = await axios.get(`https://localhost:7251/api/TeNdryshme/Porosia/ShfaqPorosineNgaID?nrFatures=${props.id}`, authentikimi);
    ...
    function handleSubmit() {
        ...
        axios.put(`https://localhost:7251/api/TeNdryshme/Porosia/PerditesoStatusinPorosis?idPorosia=${props.id}&statusi=${statusiPorosis}`, {}, authentikimi)
            .then(x => {
